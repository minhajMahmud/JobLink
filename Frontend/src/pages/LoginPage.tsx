...
import { Briefcase, Shield, Users, ArrowRight, Loader2 } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (selectedRole === "employer") { navigate("/employer"); return; }
    if (selectedRole === "admin") { navigate("/admin"); return; }
    navigate("/");
  };
      ...
      <div className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-6xl gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <section className="rounded-[2rem] border border-border bg-card p-8 shadow-card sm:p-10">
          <div className="mb-8">
            ...
            <h2 className="mt-2 font-display text-3xl font-bold text-foreground">Welcome back</h2>
            <p className="mt-3 text-sm leading-6 text-muted-foreground">
              Selecting a role auto-fills demo credentials. You can also enter your own account.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-primary px-5 text-sm font-semibold text-primary-foreground transition-opacity hover:opacity-90 disabled:opacity-60"
            >
              {isLoading ? (
